(function () {
	...
	$(".shopmenu li.shoppage li").click(function (e) {
		$.ajax({
			...
			type: "post",
			data: $(e.target).parent("li").attr("data-type") + " " + $(e.target).parent("li").attr("data-name"),
		}).success(function (req) {
			$("#money").text("Money:" + req + "$");
		});
	})
...
(function () {
	var svg = $("svg.svghddmap");
	var canvas = Raphael(svg,100,0,700);
	var circle = canvas.rect(8, 0, 500, 700);
